...
public class ModArmorItems extends ArmorItem {
    ...
    private static final Map<ArmorMaterial, List<StatusEffectInstance>> MAP =
            (new ImmutableMap.Builder<ArmorMaterial, List<StatusEffectInstance>>())
                    .put(ModArmorMaterial.BLACKSTONE.value(),
                            Arrays.asList(
                                    new StatusEffectInstance(StatusEffects
                                            .FIRE_RESISTANCE,1,1,false, false, false),
                                    new StatusEffectInstance(StatusEffects
                                            .REGENERATION,1 ,9,false,false,false)
                            )
                    )
                    .put(ModArmorMaterial.SOUL_VALLEY.value(),
                            Arrays.asList(
                                    new StatusEffectInstance(StatusEffects
                                            .FIRE_RESISTANCE,1,1,false, false, false)
                            )
                    )
                    .put(ModArmorMaterial.SPROUT.value(),
                            Arrays.asList(new StatusEffectInstance(StatusEffects
                                    .FIRE_RESISTANCE,1,1,false, false, false)))
                    .put(ModArmorMaterial.LIFE.value(),
                            Arrays.asList(new StatusEffectInstance(StatusEffects
                                    .FIRE_RESISTANCE,1,1,false, false, false))).build();

    public ModArmorItems(RegistryEntry<ArmorMaterial> material, Type type, Settings settings) {
    ...
    private void evaluateArmorEffects(PlayerEntity player) {
        for (Map.Entry<ArmorMaterial, List<StatusEffectInstance>> entry : MAP.entrySet()) {
            ...
            if (hasCorrectArmSet(player, material)){
    ...
    private boolean hasCorrectArmSet(PlayerEntity player, ArmorMaterial material) {
